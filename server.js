...
app.get("/sessies/:id/stream", async (req, res) => {
  try {
    ...
    // Set headers for SSE with VPS compatibility
    res.writeHead(200, {
    ...
    // keep-alive ping with VPS compatibility
    const interval = setInterval(() => {
      try {
        res.write(": keepalive\n\n");
        // Flush the response for VPS
        if (res.flush) res.flush();
      } catch (e) {
        clearInterval(interval);
      }
    }, 30000); // Increased to 30s for VPS stability
...
app.get("/api/license/stats", auth, async (req, res) => {
  try {
    if (!isAdmin(req)) return res.status(403).json({ error: "admin only" });
    ...
    const stats = await db.all(`
      SELECT 
        ...
        COUNT(CASE WHEN l.actief = 1 AND (l.vervalt_op IS NULL OR DATE(l.vervalt_op) >= DATE('now')) THEN 1 END) as active_licenses,
        COUNT(CASE WHEN l.klas_id IS NOT NULL THEN 1 END) as assigned_licenses,
        COUNT(CASE WHEN l.klas_id IS NULL OR l.klas_id = 0 THEN 1 END) as unassigned_licenses
      FROM licenties l
      JOIN docenten d ON l.docent_id = d.id
      GROUP BY d.id, d.naam
      ORDER BY d.naam
    `);

    res.json({ stats });
  } catch (err) {
    console.error("License stats error:", err);
    res.status(500).json({ error: "server error" });
  }
});

// Auto-assign unassigned license to klas
app.post("/api/license/assign", auth, express.json(), async (req, res) => {
  try {
    const { klas_id } = req.body;
...
app.post("/api/license/check-expiry", auth, async (req, res) => {
  try {
    if (!isAdmin(req)) return res.status(403).json({ error: "admin only" });

    const expiringSoon = await db.all(`
      SELECT l.*, d.naam as docent_naam, d.email as docent_email,
             k.naam as klas_naam, k.klascode
      FROM licenties l
      JOIN docenten d ON l.docent_id = d.id
      LEFT JOIN klassen k ON l.klas_id = k.id
      WHERE l.actief = 1 
        AND l.vervalt_op IS NOT NULL 
        AND DATE(l.vervalt_op) BETWEEN DATE('now') AND DATE('now', '+30 days')
      ORDER BY l.vervalt_op
    `);
...
// Public endpoint for cast.html - no authentication required
app.get("/sessies/:id/cast", async (req, res) => {
  try {
    const id = parseInt(req.params.id, 10);
    const sess = await db.get(
      "SELECT s.*, k.naam as klasnaam, k.klascode FROM sessies s JOIN klassen k ON k.id = s.klas_id WHERE s.id = ?",
...
// VPS-specific server configuration
